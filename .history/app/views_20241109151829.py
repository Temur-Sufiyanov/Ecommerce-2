...
def search_bar(request):
    ...
    if query:
        product = products = Product.objects.filter(name__icontains=query) | Product.objects.filter(description__icontains=query)
    else:
        messages.info(request, 'Product is not found')
    return render(request, 'app/search.html', {'query':query})
