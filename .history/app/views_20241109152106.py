...
def search(request):
    ...
    products = Product.objects.all()  # Default: Show all products if no query
    
    if query:
        # Filter products by name (case-insensitive match)
        products = products.filter(name__icontains=query)
    
    # Pagination: Display 9 products per page
    ...
    return render(request, 'app/search.html', {
        'query': query,
        'products': page_obj,  # Pass the page object (includes pagination info)
    })
